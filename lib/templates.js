  ...
  return `
    <div class='color-picker__value cp-checkboard'>
      ${showColorStringEnabled ? `
        <button title='Copy Value' name='copyValue'>
          ${icons?.copy || format.icon}
        </button>
        <input name='value' inputmode='decimal' placeholder='CSS Color' value='${any_to_hex(HSLAtoCSS(color))}' readonly>
...
export function swatches(swatches, initialSwatches) {
  const { icons, buttons: { add } } = this.settings;

  return `
    <div class='color-picker__swatches' style='--initial-len:${initialSwatches.length}'>
      <button name='addSwatch' title='${add.title}'>
        ${icons?.add || add.icon}
      </button>

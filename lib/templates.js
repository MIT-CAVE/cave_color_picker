import {any_to_hex, HSLAtoCSS, changeColorFormat} from './utils/convertColors.js'
...
export function value(color) {
  const { icons, buttons: { format }, showColorStringEnabled, colorFormats } = this.settings;
  const showSwitchIcon = colorFormats && colorFormats.length > 1;
  const initialFormat = colorFormats && colorFormats.length > 0 ? colorFormats[0] : 'hex';
  const formattedColor = changeColorFormat(HSLAtoCSS(color), initialFormat);
  ...
  return `
    <div class='color-picker__value cp-checkboard'>
      ${showColorStringEnabled ? `
        ...
        <input name='value' inputmode='decimal' placeholder='CSS Color' value='${formattedColor}' oninput="this.onColorInputChange(event)">
        ${showSwitchIcon ? `

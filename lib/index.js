...
ColorPicker.prototype = {
  ...
  setColor(value) {
    ...
    const initialFormat = this.settings.colorFormats && this.settings.colorFormats.length > 0
      ? this.settings.colorFormats[0]
      : 'hex';
    console.log(this.colorFormat);
    const currentFormat = this.colorFormat ? this.colorFormat : initialFormat;
    const formattedColor = changeColorFormat(HSLAtoCSS(hsla), currentFormat);
  
    if (this.DOM.value) {
      this.DOM.value.value = formattedColor;
    }

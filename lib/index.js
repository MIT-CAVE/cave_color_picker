...
ColorPicker.prototype = {
  ...
  setColor(value) {
    ...
    if (this.DOM.value) {
      const displayColor = this.settings.opacityEnabled
        ? this.CSSColor
        : this.CSSColor.replace(/rgba\(([^)]+)\)/, (_, colors) => {
            const [r, g, b] = colors.split(',').map(v => v.trim());
            return `rgb(${r}, ${g}, ${b})`;
          }).replace(/hsla\(([^)]+)\)/, (_, colors) => {
            const [h, s, l] = colors.split(',').map(v => v.trim());
            return `hsl(${h}, ${s}%, ${l}%)`;
          });
      this.DOM.value.value = displayColor;
    }
  },  
  ...
  build() {
    ...
    this.DOM.value = this.settings.showColorStringEnabled
      ...
      : { value: '' }; // Fallback for color updates without the text field
